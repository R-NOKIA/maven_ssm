...
<#--			<@shiro.hasPermission name="/role/clearRoleByUserIds.shtml">-->
			...
			function deleteById(ids){
				var index = layer.confirm("确定清除这"+ ids.length +"个用户的角色？",function(){
					var load = layer.load();
					$.post('${basePath}clearRoleByUserIds.do',{userIds:ids.join(',')},function(result){
						layer.close(load);
...
<#--			<@shiro.hasPermission name="/role/addRole2User.shtml">-->
			...
			function selectRole(){
				...
				var index = layer.confirm("确定操作？",function(){
					<#--loding-->
					var load = layer.load();
					$.post('${basePath}addRole2User.do',{ids:ids.join(','),userId:$('#selectUserId').val()},function(result){
						layer.close(load);
						if(result && result.status != 200){
			...
			*/
			function selectRoleById(id){
				var load = layer.load();
				$.post("${basePath}selectRoleByUserId.do",{id:id},function(result){
					layer.close(load);
