...
<#--			<@shiro.hasPermission name="/role/deleteRoleById.shtml">-->
			...
			function deleteById(ids){
				var index = layer.confirm("确定这"+ ids.length +"个角色？",function(){
					var load = layer.load();
					$.post('${basePath}deleteRoleById.do',{ids:ids.join(',')},function(result){
						layer.close(load);
...
<#--			<@shiro.hasPermission name="/role/addRole.shtml">-->
			...
			function addRole(){
				...
				if($.trim(type)==''){
					return layer.msg('角色描述不能为空。',so.default),!1;
				}
				<#--loding-->
				var load = layer.load();
				$.post('${basePath}addRole.do',{name:name,type:type},function(result){
					layer.close(load);
					if(result && result.status != 200){
